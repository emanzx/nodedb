...
pub fn dispatch(
    ...
    if upper.starts_with("CREATE USER ") {
        return Some(user::create_user(state, identity, &parts));
    }
    if upper.starts_with("ALTER USER ") {
        return Some(user::alter_user(state, identity, &parts));
    }
    ...
    if upper.starts_with("GRANT ") {
        return Some(grant::handle_grant(state, identity, &parts));
    }
    if upper.starts_with("REVOKE ") {
        return Some(grant::handle_revoke(state, identity, &parts));
    }
