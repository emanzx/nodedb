    ...
    S::Error: std::fmt::Display,
{
    ...
    loop {
        ...
        // Skip pushing if paused for auth — a token refresh is in progress.
        if client.is_push_paused_for_auth().await {
            // Try reactive token refresh if we have a provider.
            if let Some(refresh_msg) = client.initiate_token_refresh().await {
                let frame = SyncFrame::encode_or_empty(SyncMessageType::TokenRefresh, &refresh_msg);
                let mut sink_guard = sink.lock().await;
                if let Err(e) = sink_guard
                    .send(Message::Binary(frame.to_bytes().into()))
                    .await
                {
                    tracing::warn!(error = %e, "reactive token refresh send failed");
                    return;
                }
            }
            continue;
    ...
    loop {
            ...
            && let Some(refresh_msg) = client.initiate_token_refresh().await
        {
            ...
            if let Err(e) = sink_guard
                ...
                .await
            {
                tracing::warn!(error = %e, "token refresh send failed");
                return;
            }
        }
